...
function verifyPassword(password) {
  ...
  return _bcrypt2.default.compare(password, this.passwordHash).then(function (result) {
    if (!result) {
      throw new _httpErrors2.default(401, ' AUTH incorrect username or password');
    }
    return _this;
  });
}
...
function pCreateToken() {
  ...
  return this.save().then(function (account) {
    return _jsonwebtoken2.default.sign({ tokenSeed: account.tokenSeed }, process.env.STUFF_SECRET);
  });
}
